...
Original file is located at
    https://colab.research.google.com/drive/1G3s8Sx2q4qjQ1QnGzDiWnA1Hfk4Y2chB
"""

from string_splitter import splitter, spellcheck
from recognizer import raspoznavanie
...
def splitter(path_file, save_as_image=False, path=None):
    ...
    if save_as_image:
        ...
        for x in range(len(letter_list)):
            for y in range(len(letter_list[x])):
                for z in range(len(letter_list[x][y])):
                    Image.fromarray(letter_list[x][y][z]).convert('L').save(path_file + 'img' + str(x) + '.' + str(y) + '.' + str(z) + '.jpg')
...
def milinki(l):
  #l=Image.fromarray(l.reshape((278, 278, 2)))
  basewidth = 120
  wpercent = (basewidth/float(l.size[0]))
  hsize = int((float(l.size[1])*float(wpercent)))
